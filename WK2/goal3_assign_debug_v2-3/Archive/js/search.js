...
(function(){//Begin search function
	var search;//Start search
	var showMatches;// Variable initialization (DO NOT FIX ANY OF THE BELOW VAR's)
...
// Validates search query
	var validate;
	validate = function (query) {//fix the syntax errors by replacing "==" with "=" and spelled "validate" correctly
// Trim whitespace from start and end of search query
		while (query.charAt(0) === "") {//Fixed a reference error by replacing "=" with "==="
			query = query.substring(1, query.length);//initalize the while loop
	...
	var search = function(query){   //Added the opening curly braces and split the user's search query string into an array
...
var noMatch = function(){ //Variable that executes a function if no results are found
	...
	resultsDIV.innerHTML = html; //Allows the HTML and innerHTML to be displayed
...
}; //Closing curly brace for function and
