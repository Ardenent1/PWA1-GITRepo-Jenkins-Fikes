...
// Trim whitespace from start and end of search query using charAt method and substring
		while (query.charAt(query.length - 1) === " ") { //begin while loop to see if search query is empty. charAt() method to return the character at the specified index in a string should be equal to a space
			query = query.substring(0, query.length); //extract the characters from a string, between two specified indices (1 is "start" and query.length, or the length of the string entered, is "end")
...
// Check that matches were found, and run output functions
		...
		} else {//ELSE if there is a match, ie results.length>= 1, will be placed into the showMatches
			showMatches(results);
		}
...
// Put "No Results" message into page (DO NOT FIX THE HTML VAR NOR THE innerHTML)
	var noMatch = function () {//create function that displays "No Results" into HTML if the above statement is met
		var html = '' +
				'<p>No Results found.</p>' +
				'<p style="font-size:10px;">Try searching for "JavaScript".  Just an idea.</p>'
			;
		resultsDIV.innerHTML = html;//DOM sets the inner HTML code of the HTML element to dynamically change the page.
	};

// Put matches into page as paragraphs with anchors
	showMatches = function (results) {//when above ELSE statement for the show matches are met, applu function to the show user results of their search
